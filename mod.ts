import type { Tquery } from "./types.d.ts";
import * as util from "node:util";
import { escape, unescape } from "@std/html";

export function alertTelegram(config: { host: string, telegram_bot: string, telegram_alert_chat_id: string }, data: any, retry = true): Promise<Response | false> {
...
export function removeHTML(s: any): string {
	if (s === 0) return s;
	if (!s) return '';
...
export function argsToObject(args: string[]): Tquery {
	var query: Tquery = {};
	args.forEach((arg) => {
		var [k, v] = arg.split('=');
		query[k] = v ?? true;
...
export function addQuery(query: Tquery, params: URLSearchParams | FormData) {
...
export function randomString(length = 4, options = { case: 'lower', numbers: false, symbols: '' }): string {
	options.case ||= 'lower';
...
*/
export function objectDifferentKeys(o1: any, o2: any): string[] {
	var r = [];
...
export function objectUnion(obj: any, newObj: any) {
...
export function readJsonSync(filename: string, ingoreNonexisting = false): object {
	var res: any;
	try {
...
export function resolveObject(obj: object): Promise<object> {
	var ps: Promise<any>[] = [],
		resolveValues = (obj: any) => {
		...
		resolveOne = (value: any, key: string | number, obj: any) => {
			if (value?.then) {
				ps.push(value.then((value: any) => obj[key] = value));
...
var ts: any = {};
export function mt(label = '', reset = false): string {
